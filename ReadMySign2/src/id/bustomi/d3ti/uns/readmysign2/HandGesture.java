...
public class HandGesture {
	...
	String featureExtraction(Mat img, int label){
		...
		if ((detectIsHand(img))) {			
			...
			Log.i("Panjang Feature", ""+features.size());
			
			if (features.size() == 0) {
				
				double huMoments[];
				double momentsForMatchShapes[];
				Moments moments = new Moments();
				moments = Imgproc.moments(contours.get(0));
				Mat huMomentsMat = new Mat();
				Imgproc.HuMoments(moments, huMomentsMat);
				huMoments = new double[(int) huMomentsMat.total() * huMomentsMat.channels()];
				huMomentsMat.get(0, 0, huMoments);
				// System.out.println("VELIKOST: " + huMoments.length);
				momentsForMatchShapes = new double[huMoments.length];
				for (int j = 0; j < huMoments.length; j++) {
					momentsForMatchShapes[j] = Math.signum(huMoments[j]) * Math.log(huMoments[j]);
					features.add(huMoments[j]);
				}
				Log.i("Moments", moments.toString());
				Log.i("Hu Moments", Arrays.toString(huMoments));
				Log.i("Panjang Hu Moments", Arrays.toString(huMoments));
